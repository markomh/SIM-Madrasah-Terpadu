...
- `jadwal_pelajaran` *(diperbaiki lagi — revisi kedua, menambahkan dukungan Team Teaching dan Fasilitas)*: `id_jadwal` (PK), `id_rombel` (FK — tenant diwarisi lewat sini, lihat Bab 9P), `id_pegawai` (FK, guru pengajar **utama**), `id_mapel` (FK), `id_fasilitas` (FK nullable ke `fasilitas`, Bab 9Q — diisi hanya kalau sesi ini memakai fasilitas kapasitas tunggal seperti Lab/GOR), `semester` — "Ganjil"/"Genap", `hari`, `jam_mulai`, `jam_selesai`.
  - **Aturan bentrok direvisi jadi tiga lapis, bukan satu** (SRS Bab 10 poin 3, revisi kedua):
    1. Bentrok **guru**: `id_pegawai` + `hari` + `jam_mulai` + `semester` harus unik — **kecuali** baris tersebut adalah bagian dari sesi Team Teaching yang sama (lihat `jadwal_pengajar_tambahan` di bawah — guru tambahan **tidak** dicatat sebagai baris `jadwal_pelajaran` terpisah, jadi tidak pernah bentrok dengan baris utamanya sendiri).
    2. Bentrok **rombel** *(baru — celah yang sebelumnya tidak tertutup)*: `id_rombel` + `hari` + `jam_mulai` + `semester` harus unik — satu rombel tidak bisa punya dua sesi KBM berbeda bersamaan. **Team Teaching bukan pengecualian aturan ini** — karena Team Teaching tetap **satu** baris `jadwal_pelajaran` (satu sesi, satu rombel, guru tambahan dicatat terpisah, bukan baris jadwal kedua).
    3. Bentrok **fasilitas** *(baru)*: kalau `id_fasilitas` terisi, kombinasi `id_fasilitas` + `hari` + `jam_mulai` + `semester` harus unik — **hard constraint**, sistem menolak total (bukan peringatan), karena satu Lab/GOR fisik memang tidak bisa dipakai dua rombel bersamaan.
  - Validasi tambahan: `id_rombel`, `id_pegawai`, `id_fasilitas` (kalau diisi) wajib berasal dari `id_madrasah` yang sama.
- `jadwal_pengajar_tambahan` *(baru — mendukung Team Teaching)*: `id_pengajar_tambahan` (PK), `id_jadwal` (FK ke `jadwal_pelajaran`), `id_pegawai` (FK, guru pendamping). Satu `jadwal_pelajaran` boleh punya lebih dari satu baris di sini (P5RA, praktikum, pendalaman materi dengan >1 guru). **Guru di sini tidak divalidasi bentrok terhadap `jadwal_pelajaran.id_pegawai` lain** dengan cara yang sama seperti guru utama — tetap dicek jangan sampai guru pendamping ini sendiri sedang mengajar sesi lain di waktu yang sama (bentrok guru tetap berlaku untuknya, hanya saja dia bukan "pemilik" baris `jadwal_pelajaran`).
- `fasilitas` *(baru)*: `id_fasilitas` (PK), `id_madrasah` (FK — **wajib**), `nama_fasilitas` (mis. "Lab IPA", "Lab Komputer", "GOR", "Ruang Keagamaan/Masjid"), `kapasitas_rombel_bersamaan` (integer, default 1 — hampir semua fasilitas ini kapasitas tunggal, tapi field ini disiapkan kalau suatu saat ada fasilitas berkapasitas lebih dari satu rombel).
- `ketidaktersediaan_guru` *(baru)*: `id_ketidaktersediaan` (PK), `id_pegawai` (FK), `hari`, `jam_mulai`, `jam_selesai` (nullable — kalau kosong berarti sepanjang hari), `id_tahun` (FK), `alasan` (teks — mis. "kuliah", "tugas tambahan manajerial", "dinas luar"), `jenis` (enum: **"Preferensi Guru Honorer"**, **"Dikunci Waka Kurikulum"**), `dikunci_oleh` (FK nullable ke `pegawai`, diisi kalau `jenis` = "Dikunci Waka Kurikulum").
  > **Perbedaan penegakan berdasarkan `jenis`** (Bab 10, aturan baru): entri berjenis **"Dikunci Waka Kurikulum"** adalah **hard constraint** — sistem menolak total penjadwalan yang bentrok dengan slot ini, karena itu keputusan sadar pihak berwenang. Entri berjenis **"Preferensi Guru Honorer"** adalah **soft/peringatan** — sistem memperingatkan tapi mengizinkan Admin melanjutkan dengan alasan, karena ini preferensi, bukan larangan mutlak.
- `alokasi_jtm_kurikulum` *(baru — formalisasi SK Pembagian Tugas)*: `id_alokasi` (PK), `id_pegawai` (FK), `id_mapel` (FK), `id_rombel` (FK), `id_tahun` (FK), `jtm_per_minggu` (integer, sesuai SK Pembagian Tugas). **Total baris `jadwal_pelajaran`** untuk kombinasi guru+mapel+rombel+semester yang sama **seharusnya berjumlah sama** dengan `jtm_per_minggu` di sini — penyimpangan memicu peringatan (bukan penolakan) di halaman penjadwalan, supaya linearitas sertifikasi (TPG) terjaga tapi Admin tetap punya keleluasaan untuk kondisi mendesak (SRS Bab 10, aturan "Fleksibilitas Darurat").
