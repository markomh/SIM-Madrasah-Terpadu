...
| 2026-09-01 | Konsolidasi Kontrak / Semua | Menambahkan method `getAllKonflik` (`/jadwal/konflik`) pada `jadwal.api.ts`, mengubah penanganan `.catch(() => [])` pada context otentikasi menjadi indikasi error UI eksplisit (`isConnectionError`), dan mengonfirmasi penyelesaian pemecahan service per domain. | Eksekusi tindak lanjut dari Task 23 KONSOLIDASI (Penertiban SSoT). Menghentikan penelanan error secara diam-diam dan menjaga sinkronisasi kontrak endpoint frontend dan backend. |
...
| 2026-08-12 | Arsitektur & Otorisasi / Audit Traceability | Audit Traceability (task_09 & task_10) mendokumentasikan konsistensi antara SRS v2, CONTRACT_MATRIX, FRONTEND.md, backend.md, dan codebase `src/`. | Kesiapan arsitektural sebelum Backend Laravel Tahap 2 dimulai. |
...
| 2026-08-17 | Audit Enterprise & Rekonsiliasi Kontrak | Audit Enterprise ([task_17.md](file:///d:/titip%20video%20hp%20Vivo/APLIKASI/Sim%20Madrasah/SIM-Madrasah-Terpadu/doc/task_17.md)): Menambahkan rute API backend rumpang (`GET/PUT /pengaturan`, `GET /jadwal/konflik`, `DELETE /ekstrakurikuler/{id}/anggota/{id_anggota}`), penyelarasan token design system FE (`primary`, `border`, `muted`), refactoring komponen UI primitif `<Button>`, dan pembakuan `StatusSurat` type safety. | Menghilangkan gap rute 404 pada mode `USE_MOCK=false`, menjamin 100% konsistensi visual design system, serta memperketat type safety. |
| 2026-08-31 | SSoT Consolidation | Otorisasi baru (ProfilMadrasahPolicy & TemplateSuratPolicy), default USE_MOCK aman, dekomposisi remaining-api.ts, dan audit endpoint mismatch (/pindah-rombel/massal di FE tidak ada di BE, /jadwal/konflik di BE tidak dipakai di FE). | Mematuhi instruksi task_23_KONSOLIDASI.md untuk menutup celah otorisasi, penertiban SSoT, dan audit rute. |
